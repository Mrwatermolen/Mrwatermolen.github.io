...
但是也可以不满秩，在`Otcave`中可以使用`pinv`求
...
$\pmb{x_i}=[x_{i1},x_{i2},\cdots,x_{in}]^T$代表第i个输入数据，$x_{ij}$代表第i个输入数据的第j个特征。其中$x_i0 = 1$
$$
\pmb{X}=\left[
    \begin{matrix}
    x_{10}&x_{11}&\cdots&x_{1n}\\
...
$$
P(y=1|\pmb{x};\pmb{\theta})=h_{\pmb{\theta}}(\pmb{x}) ;\quad P(y=0|\pmb{x};\pmb{\theta})=1-h_{\pmb{\theta}}(\pmb{x})
$$

又：
$$
y=\frac{1}{1+e^{-x}}\rightarrow x=\ln{\frac{y}{1-y}}
$$
...
\end{aligned}
$$
很明显$y_i$与$h_{\pmb{\theta}}(\pmb{x_i})$为0-1分布
